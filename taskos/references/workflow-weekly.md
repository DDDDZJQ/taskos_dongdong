...
| 信息 | 用途 | 如果缺失 |
|------|------|----------|
| 本周精力状态（high/normal/low） | 决定三档时间额度的浮动比例 | 主动问 |
...
### ⚙️ 三档排期上限：时间制（v1.6.0，规则层）

三档排期上限按**时间**（est 估时累加）控制，不再按条数：

**硬约束**：
- 本周所有档位排期任务的 est 之和 ≤ `weekly_est_limit`（周总上限，默认 32h，用户可改）
- must 档 est 之和 ≤ `weekly_tier_limits.must`（默认 15h）
- should 档 est 之和 ≤ `weekly_tier_limits.should`（若仍为 null：优先走下方「首次引导确认」；用户当下不愿定，则本次临时按建议初值 8h 校验，source 仍保持 default 待下次再问）
- could 档**不校验时间上限**（纯弹性），仅受周总上限兜底

**自洽**：must(15) + should(8) = 23h < 周总(32h) ✓；could 可用空间 = 周总 − 实际 must − 实际 should。

**💡 精力浮动（指导层，AI 临场判断，可偏移 10~20%）**：INDEX 存的是满精力（high）基准；精力低的周按比例临场建议下调，不查死表：

| 精力 | must 参考 | should 参考 | could |
|------|-----------|-------------|-------|
| high | 基准（如 15h） | 基准（如 8h） | 不限 |
| normal | 约基准 ×0.7 | 约基准 ×0.7 | 不限，收着点 |
| low | 约基准 ×0.4 | 约基准 ×0.4 | 建议不排（could 始终无上限，仅建议把精力留给 must） |

**💡 首次引导确认**（`tier_limits_source == default` 且本次用到档位上限时）：
- AI 一次性确认（自然语言）："这周开始用时间额度控制排期了。核心任务（must）周上限我先按 15h，常规档（should）建议 8h，可选档（could）不设上限——总上限 32h。这几个数都能随时改，按这个来吗？"
- 用户确认/调整后：写入 `weekly_tier_limits`（should 的 null 用确认值填上）、`tier_limits_source → manual`、走 journal
- 用户之后任何时候说"把 must 改成 12h"等 → 直接改 INDEX 字段，走 journal，无需再引导

**⚙️ est 缺失处理**（时间制落地命门）：
1. 要进 must/should 档的任务若 `est` 为空 → AI 先按标题推断粗估，或问一句"这条大概要多久？"
2. 用户跳过/无法估 → 该任务 est 暂记 1h（沿用无 est 按 1h 惯例），如实计入档位合计
3. 某档因大量缺 est 逼近上限 → 提示"有 N 条没估时，按 1h 兜底算的，实际可能更多"
4. could 不强制补 est、不校验子上限；但无 est 的 could 任务仍按 1h 计入**周总** est_total（占周总闸，不占子上限）

> **⚠️ 两个口径别混用**：本段的 `weekly_est_limit`(32h) / 三档时间额度，单位是**排期任务 est 总和**（含深度+浅层+ritual 等全部任务耗时）；而下方「工作量校准」的"业余深度 6~12h/周"是**纯高认知深度工作**的研究参考。两者口径不同，不可直接比较（不能因 must=15h > 12h 就判定过载）。校准时用甜点值（个人历史 est_total 中位数）作主锚，深度参考仅作背景。
...
- 用户接受 → 恢复周模式：把三档时间额度大幅压低（must 砍到基准约 1/3、should 大幅压低、could 不排）；指导层临场判断，不写死数字
- 恢复周额外行为：
...
| 档位 | 上限（时间制，见上方三档时间制段） | 来源 |
|------|---------------------|------|
| must | est 之和 ≤ weekly_tier_limits.must（默认 15h） | core 关键任务 + 🔴 normal 项目 |
| should | est 之和 ≤ weekly_tier_limits.should（建议 8h） | core 延伸 + normal 常规 + 本周 ritual |
| could | 不设上限（纯弹性），仅受周总上限兜底 | side 项目任务 |
...
- 用户说"我自己排" → 退回传统逐条模式
- 草案各档 est 累加不超对应时间上限（must/should），总和不超周总上限；could 不限
- active 任务池 < 5 条时跳过草案
...
排期完成并 journal [done] 之后，如无其他事项则告知用户本周计划已就绪。
