...
data_schema: 1.8.0                     # 当前数据 schema 版本（v1.6.0 三档时间制 + v1.7.0 习惯系统 + v1.8.0 Streak 退役）
...
> v1.8.0：`rewards/streaks.md` 已退役（连续纪录子系统并入习惯系统）。

### Ledger 行格式

```json
{"id":"wc-YYYYMMDD-NNN","type":"earn|spend|adjust","amount":1,"reason":"xxx","source":"direct|bounty|challenge|habit-graduate","bounty_id":null,"challenge_id":null,"date":"YYYY-MM-DD"}
```
...
| `amount` | integer | ✅ | 正整数 |
| `reason` | string | ✅ | 具体原因 |
| `source` | string | earn 时必填 | `"direct"` / `"bounty"` / `"challenge"` / `"habit-graduate"` |
| `bounty_id` | string | 否 | 关联悬赏 ID |
| `challenge_id` | string | 否 | 关联挑战 ID |
| `date` | string | ✅ | YYYY-MM-DD |

> v1.8.0：earn source 枚举删除 `streak`（连续纪录退役）；历史 ledger 行若含已废弃的 `streak_id` 字段不报错（向后兼容），新行不再写。
...
| `auto_detect` | string \| null | 否 | `"weekly_must_clear"` / `"project_done:项目名"` / null |
...
> v1.8.0：删除 `active_streaks` 字段（连续纪录退役）。
...
> v1.8.0：删除连续纪录 ID `s-YYYYMMDD-NNN`（Streak 退役）。
...
- 与 area ritual 按目的划界（详见 workflow-habit.md）；v1.8.0 起为唯一连续打卡引擎（许愿卡 Streak 已退役）
